...
pub fn make_check_error_at_cursor(
    ...
    Callback::new(move |(line, col): (u32, u32)| {
        ...
        if let Some(diag) = diag_opt {
            web_sys::console::log_1(&wasm_bindgen::JsValue::from_str(&format!(
                "[check_error_at_cursor] Matching diagnostic found: {}",
                diag.message
            )));

            let current = active_error.get_untracked();
            if let Some((curr_diag, _, _)) = &current {
                if curr_diag.message == diag.message
                    && curr_diag.range.start.line == diag.range.start.line
                {
            ...
            spawn_local(async move {
                ...
                {
                    web_sys::console::log_1(&wasm_bindgen::JsValue::from_str(&format!(
                        "[check_error_at_cursor] API call success. Got {} suggestions.",
